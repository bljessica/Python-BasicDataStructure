...
class MinPriorityqueue:
    ...
    # def __init__(self, max_size):
    #     self.MAX_SIZE = 10 #队列最大容量
    #     self.queue = [sys.maxsize for i in range(self.MAX_SIZE)]#初始化队列
    #     self.size_now = 0
    def __init__(self, edges):
        self.edges = edges
        self.size_now = len(edges)
    ...
    def decrease_key(self, i, key):
        '''减小某个节点的值'''
        if self.edges[i].weight < key:
            return
        while i > 0 and self.edges[self.parent(i)].weight > self.edges[i].weight:
            self.swap(i, self.parent(i))
            i = self.parent(i)
    ...
    def insert(self, edge):
        ...
        self.size_now += 1
        self.decrease_key(self.size_now - 1, edge.weight)
        # if self.size_now == self.MAX_SIZE:
        #     print('队列已满')
        #     return
        # self.queue[self.size_now] = ele
        # self.size_now += 1
        # self.decrease_key(self.size_now - 1, ele)
    ...
    def delete_min(self):
        ...
        if self.size_now == 0:
            return
        # self.swap(0, self.size_now - 1)
        res = self.edges.pop(0)
        self.size_now -= 1
        self.adjust_heap(0, self.size_now - 1)
        return res
        # if self.size_now == 0:
        #     print('队列为空')
        #     return
    ...
    def adjust_heap(self, i, length):
        ...
        k = 2 * i + 1  # 左儿子
        while k < length:
            if k + 1 < length and self.edges[k].weight > self.edges[k + 1].weight:
                k += 1  # 指向右儿子
            if self.edges[k].weight < tmp.weight:  # 子节点小于父节点
                self.edges[i] = self.edges[k]
                i = k  # 继续往下
            else:
                break
            k = 2 * k + 1
        self.edges[i] = tmp  # 放到最终位置
        # tmp = self.queue[i]
        # k = 2 * i + 1 #左儿子
        # while k < length:
        #     if k + 1 < length and self.queue[k] > self.queue[k + 1]:
        #         k += 1 #指向右儿子
        #     if self.queue[k] < tmp:#子节点小于父节点
        #         self.queue[i] = self.queue[k]
        #         i = k  # 继续往下
        #     else:
        #         break
